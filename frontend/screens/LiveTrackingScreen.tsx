...
const SOCKET_URL = "http://192.168.1.7:4000"; // Replace with your backend IP
const GEOFENCE_RADIUS = 200; // meters
const MAX_ROUTE_POINTS = 100; // max polyline points
...
export default function LiveTrackingScreen() {
  ...
  // Request location permissions & get initial location
  useEffect(() => {
    (async () => {
          ...
          if (geofenceOrigin) {
            ...
            if (distance > GEOFENCE_RADIUS && !outsideGeofence) {
              ...
              socketRef.current?.connected &&
                socketRef.current.emit("geofence_breach", { location: p });
              await Notifications.scheduleNotificationAsync({
                content: {
                  title: "🚨 Geofence Alert",
                  body: "You exited the safe zone!",
                  sound: true,
                },
                trigger: null,
              });
  ...
  const triggerPanic = async () => {
    ...
    await Notifications.scheduleNotificationAsync({
      content: {
        title: "⚠️ Panic Alert",
        body: "User triggered a panic alert!",
        sound: true,
      },
      trigger: null,
    });
      ...
      {geofenceOrigin && (
        <View style={styles.statusBanner}>
          <Text
            style={{
              fontWeight: "bold",
              color: outsideGeofence ? "red" : "green",
            }}
          >
          ...
          <Text>
            ...
            {distanceFromOrigin !== null ? `${distanceFromOrigin} m` : "…"}
          </Text>
        </View>
      )}
